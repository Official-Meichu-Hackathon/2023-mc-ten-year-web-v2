...
export default function YearPage({ params }) {
    ...
    useEffect(() => {
        ...
        if (data && !data.invalid && !data.notFound) {
        ...
        <div className="my-16 petite:my-16 md:my-10">
            ...
            <main>
                <article className="grid gap-y-32 [&_h2]:mb-16">
                    ...
                    <Staffs year={params.year} logo={logo} staffs={data.staffs} />
                </article>
            </main>
        </div>
    );
                ...
                {topics.map((topic, index) => (
                    <li key={index}>
                        <Card className="place-items-center md:grid-flow-col md:grid-cols-[auto_1fr]" nue="sm">
                            {(width >= breakpointMD) && (
                                <CardImg className="h-28 md:h-56 aspect-square" img={CompanyTemp} contain />
                            )}
                            <CardContent title={topic.company}
                                         content={"**主題**：" + topic.title + "\n\n**說明**：" + topic.description}
                                         moreInfo="下載 PDF" link={topic.pdf} />
                        </Card>
                    </li>
                ))}
            </ul>
...
function Staffs({ year, logo, staffs }) {
    return (
